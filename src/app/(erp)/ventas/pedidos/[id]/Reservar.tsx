...
/**
 * La fila de cobertura completa: los datos de la línea y, debajo, el panel
 * para apartar.
 *
 * POR QUÉ LA FILA ENTERA Y NO SOLO UN BOTÓN
 * El panel mide más de lo que cabe en una celda. Metido en la última columna,
 * estiraba esa columna y empujaba todas las demás: la tabla dejaba de leerse
 * justo cuando más falta hacía. Ahora el panel va en una fila propia que ocupa
 * todo el ancho, debajo de la línea a la que pertenece.
 */
export function FilaCobertura({
  pedidoLineaId,
  producto,
  puede,
  columnas,
  ...
  producto: string;
  puede: boolean;
  /** Cuántas columnas tiene la tabla, para que la fila del panel las abarque. */
  columnas: number;
  /** Las celdas de datos de la línea, ya renderizadas en el servidor. */
        ...
        <td>
          {puede && (
            ...
            >
              ...
              {cargando ? 'Buscando…' : opciones ? 'Cerrar' : 'Apartar stock'}
            </button>
          )}
        </td>
      </tr>
              ...
              <div className="reservar">
                ...
                {opciones.candidatos.length === 0 ? (
                  <p className="reservar-vacio">
                    ...
                    Esta línea no se puede cubrir hasta que entre producción, se libere una
                    reserva vencida o calidad levante un bloqueo.
                  </p>
                ) : (
                  <>
                          ...
                          {opciones.candidatos.map((l) => {
                            ...
                            return (
                              <tr key={`${l.lote_id}-${l.almacen_id}`}
                                ...
                                <td className="num"
                                    style={{ color: l.meses >= 12 ? 'var(--atencion)' : undefined }}>
                                  {cifra(l.meses, 1)}
                                </td>
                  ...
                  <div className="reservar-confirma">
                    <div className="reservar-campo">
                      ...
                      <small>
                        ...
                        {' · '}
                        {/* Se enseña la conversión para que nadie la haga a mano */}
                        {Math.max(1, Math.ceil(kilos / elegido.kg_por_bulto))} bultos de{' '}
                        {cifra(elegido.kg_por_bulto, 1)} kg
                      </small>
                    </div>
